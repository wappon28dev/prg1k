...
int set_quake_filtered_by_lat(const Quake *quakes, const int quakes_len, const Range lat_range, Quake *filtered_quakes)
{
  int idx = 0;
  for (int i = 0; i < quakes_len; i++)
  {
    double lat = quakes[i].point.lat;
    if (lat_range.min <= lat && lat < lat_range.max)
    {
      filtered_quakes[idx] = quakes[i];
      idx++;
    }
  }
  return idx;
...
int main(int argc, char *argv[])
{
  ...
  {
    Range lat_range = {i, i + step};
    Quake filtered_quakes[quakes_len];
    int filtered_quakes_len = set_quake_filtered_by_lat(quakes, quakes_len, lat_range, filtered_quakes);

    printf("| [%2d, %2d)\t| %d\n", i, i + step, filtered_quakes_len);
  }
