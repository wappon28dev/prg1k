...
int load_members_from_file(const char *file_name, Elem **head)
{
  ...
  {
    *head = prepend(*head, parse_from_str(line));
    lines_len++;
  }
...
void write_members_to_file(const char *file_name, const Elem *head)
{
  ...
  while (current != NULL)
  {
    fprintf(fp, "%s,%d,%d\n", current->member.name, (int)current->member.height, (int)current->member.weight);
    current = current->next;
  }
...
int main(int argc, const char *argv[])
{
  ...
  Elem *head = NULL;
  int members_len = load_members_from_file(file_name, &head);
  ...
  write_members_to_file(file_name, head);
