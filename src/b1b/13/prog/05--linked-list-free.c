...
Elem *add_melem(Elem *head, Member data)
{
...
int main(int argc, const char *argv[])
{
  ...
  head = add_melem(head, (Member){"Hyodo", 1.7, 70});
  head = add_melem(head, (Member){"Akihisa", 1.8, 60});
  head = add_melem(head, (Member){"nasubi", 1.6, 80});
