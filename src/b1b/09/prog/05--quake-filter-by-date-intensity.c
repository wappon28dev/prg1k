...
int set_filtered_quakes_by_date_or_later(const Quake *quakes, const int quakes_len, const Date date,
                                         Quake *quakes_filtered)
{
...
int set_filtered_quakes_by_intensity_or_higher(const Quake *quakes, const int quakes_len, const Intensity intensity,
                                               Quake *quakes_filtered)
{
  int idx = 0;
  for (int i = 0; i < quakes_len; i++)
  {
    if (quakes[i].intensity.key >= intensity.key)
    {
      quakes_filtered[idx] = quakes[i];
      idx++;
    }
...
int main(int argc, char *argv[])
{
  ...
  int quakes_filtered_len = set_filtered_quakes_by_date_or_later(quakes, quakes_len, target_date, quakes_filtered);

  Intensity target_intensity = get_intensity_from_key(i_4);
  quakes_filtered_len = set_filtered_quakes_by_intensity_or_higher(quakes_filtered, quakes_filtered_len,
                                                                   target_intensity, quakes_filtered);

  printf("#Quakes (>= %s && >= %s): %d/%d\n", date_to_str(target_date), target_intensity.name, quakes_filtered_len,
         quakes_len);
