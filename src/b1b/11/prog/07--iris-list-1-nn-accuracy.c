...
int main(int argc, const char *argv[])
{
  ...
  set_iris_list_sliced(iris_list_shuffled, 0, iris_list_test_len, iris_list_test);

  Iris iris_list_train[iris_test_train_len];
  set_iris_list_sliced(iris_list_shuffled, iris_list_test_len, iris_list_len, iris_list_train);
  ...
  for (int i = 0; i < iris_list_test_len; i++)
  {
    ...
    double calculated_distance;
    set_inferred_iris_class(iris_list_test[i], iris_list_train, iris_test_train_len, inferred_class,
                            &calculated_distance);

    if (strcmp(inferred_class, iris_list_test[i].class) == 0)
    {
      correct_count++;
    }
  ...
  printf("Accuracy parcentage: %lf%%\n", (double)correct_count / iris_list_test_len * 100);
